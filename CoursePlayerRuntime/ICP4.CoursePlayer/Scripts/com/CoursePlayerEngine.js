...
var IcoBookMark = "#IcoBookMark";
var IcoBookMarkDs = "#IcoBookMarkDs";
var IcoHelp = "#IcoHelp";
...
var timer = "#timer";
var assessmentTimer = "#assessmentTimer";
var footer = "#footer";
var overlay = "#overlay";
var dialog = "#dialog";
   ...
   //STOP
    ...
    this.init = function() {
        ...
        if (courseApproval == true) {
            ...
            document.onmousedown = function(e) { if (!$("#examIdleTimerDialogue").is(":visible")) { if (navigator.appName == "Netscape") { if (e.which == 1) { renderEngine.isIdleTimerReset = true; cpEngine.resetTimer() } } else { if (event.button == 1) { renderEngine.isIdleTimerReset = true; cpEngine.resetTimer(); } } } };
            document.onkeypress = function() { if (!$("#examIdleTimerDialogue").is(":visible") || event.srcElement.id == "btnResume") { renderEngine.isIdleTimerReset = true; cpEngine.resetTimer(); } };
            //document.onkeydown=function(){if (   !$("#examIdleTimerDialogue").is(":visible")  ||  event.srcElement.id == "btnResume"){  cpEngine.resetTimer(); }};
            window.onscroll = function() { if (!$("#examIdleTimerDialogue").is(":visible")) { renderEngine.isIdleTimerReset = true; cpEngine.resetTimer(); } };
    ...
    this.getQueryString = function() {
        ...
        //var _queryString = "SESSION=c42e1c89-9078-4ee7-a170-04d49a3fba68&ts=1400238746558"; // Dual Signer

        //var _queryString = "SESSION=167bc39e-5d17-4c2a-ba6c-714b076a1279&ts=1453215715648";
    ...
    this.initHandlers = function() {
        ...
        $(logoutbutton).find("a").bind('click.namespace', function() {
            ...
            if (QuestionType.indexOf("MSSQ") >= 0) {
                ...
                */
                ...
                for (j = 0; j < alen; j++) {
                    ...
                    if ($(evalQuestions).find('DIV').eq(i).find('input').eq(j).attr('checked')) {
                        /*
                ...
                if ($(evalQuestions).find('DIV').eq(i).find('textarea').attr('value') != undefined)
                    TEXTValue = $(evalQuestions).find('DIV').eq(i).find('textarea').attr('value');
                else
                    TEXTValue = $(evalQuestions).find('DIV').eq(i).find('input').attr('value');
                // alert(TEXTValue);
...
function soundOnOffHandler(val) {
    if (val == 'Off') {
        $('.slider_bar').slider('disable');
        setVolume(0);
    } else if (val == 'On') {
        $('.slider_bar').slider('enable');
        setVolume($('.slider_bar').slider("value"));
    }
}
