...
function RenderEngine() {
    ...
    this.ShowSeatTimeExceed = function(ShowSeatTimeExceed) {
        ...
        $(controlPanel).find("#IcoOptions").hide();
        $(controlPanel).find("#IcoOptionsDs").show();
    ...
    this.CourseMaterialRendering = function(courseMaterialObject) {
        ...
        {
             $(spMaterialh3).attr('style','display:none');
        }
        else
        {	
             $(spMaterial).html(courseMaterialList);
        }
    ...
    this.ShowEOCInstructions = function(EOCInstructionObj) {
        ...
        htmlData += "<div class='EOCInstructions'><h2>" + EOCInstructionObj.EOCInstructions.HeadingEOCInstructions + "</h2></div>";
    ...
    this.ShowGlossaryItem = function(glossaryItemObject, glossaryTerm) {
        //glossaryItemObject.GlossaryDetail.GlossaryID;
        //glossaryItemObject.GlossaryDetail.GlossaryDefinition
        if (!isGlossaryDialogueOpen) {
            jQuery().ready(function() {
                $(overlay).css({ "opacity": "0.7" });
                $(overlay).fadeIn("slow");
                $(glossaryDialogue).fadeIn("slow");

                $(glossaryDialogue).find("h2").html(glossaryItemObject.GlossaryDetail.HeadingGlossaryBoxTitle)
                $(glossaryDialogue).find("dt").eq(0).html(glossaryItemObject.GlossaryDetail.HeadingGlossaryTerm);
                //Modified by Danish Khan
                $(glossaryDialogue).find("dd").eq(0).html(replaceSpacesAndLineBreaks(glossaryTerm));
                $(glossaryDialogue).find("dt").eq(1).html(glossaryItemObject.GlossaryDetail.HeadingGlossaryDefinition);
                //$(glossaryDialogue).find("dd").eq(1).html(InsertEscapeSequence(replaceSpacesAndLineBreaks(glossaryItemObject.GlossaryDetail.GlossaryDefinition),"'")); // Modified by Mustafa On Sep 2nd 2009
                //Modified by Danish Khan
                //$(glossaryDialogue).find("dd").eq(1).html(glossaryItemObject.GlossaryDetail.GlossaryDefinition.replace(new RegExp("<","g"),"&lt;").replace(new RegExp(">","g"),"&gt;").replace(new RegExp("'","g"),"&#39").replace(new RegExp("\"","g"),"&quot;")); 
                $(glossaryDialogue).find("dd").eq(1).html(replaceSpacesAndLineBreaks(glossaryItemObject.GlossaryDetail.GlossaryDefinition));

                $(glossaryDialogue).find("button").html(glossaryItemObject.GlossaryDetail.ButtonText);
                isGlossaryDialogueOpen = true;
            });

            $(glossaryDialogue).find("button").bind('click.namespace', function() {
                $(glossaryDialogue).find("button").unbind('click.namespace');
                $(glossaryDialogue).fadeOut("slow");
                $(overlay).fadeOut("slow");
                resetCPIdleTimer();
                var target = $('#glossaryDialogue dl .deflist').get(0); //the scrolled div 
                target.scrollTop = 0; //reset scroll Top
                isGlossaryDialogueOpen = false;
                return false;
            });
        } // if end
    ...
    this.ShowCustomMessageRendering = function(customObj) {
        if (customObj.CustomMessage.CustomMessageType == "SessionEnd") {
            ...
            var html = "";
            html += "<div id='sessionexpire_main_container'>";
            html += "<div id='frame_body'>";
    ...
    this.ShowQuestion = function(questionObject) {
        ...
        jQuery().ready(function() {
            ...
            $('#toogleFlag').hide(); //Added By Abdus Samad LCMS-12105
        ...
        if (allowSkipping == true) {
            if (questionObject.AssessmentItem.PolicyEachQuestionAnswered != true) {
                $('#toogleFlag').css('display', 'block'); //Added By Abdus Samad LCMS-12105
    ...
    this.ShowCourseEvaluationQuestions = function(questionObject) {
        ...
        var courseEvalHTML = "";
        courseEvalHTML = courseEvalHTML + '<div id="EvalQuestions"><h1 style="padding-left: 20px; padding-top: 30px;">' + questionObject.CourseEvaluation.CourseEvaluationName + '</h1>';
        ...
        var errorDiv = "<div style='float:left;clear:left;display:none;' id='message_box' ><img src='images/close.png' style='float: right;cursor: pointer;' id='close_message'><div class='width40pct'><div class='message_box_align' style='float:left'><img height='14' width='14' alt='' src='images/exclamation.gif'></div><div class='message_box_align' style='float:left'>&nbsp;" + courseEval_Req_Validation + "</div></div></div>";
        courseEvalHTML = courseEvalHTML + "</div><br>";
        courseEvalHTML = errorDiv + courseEvalHTML;
    ...
    this.ShowProctorAuthenticationResult = function(proctorAuthenticationResultObject) {
        ...
        if (msg == "") {
            ...
            $("#proctor_login_screen").find("#proctorErrorLabel").html(msg);
        }
    }
    ...
    this.ShowProctorLoginScreen = function(proctorLoginScreenObject) {
        ...
        $(BackbuttonEn).hide();
        $(BackbuttonDs).show();
        $("#proctorLoginErrorMessage").hide();
        $("#proctorPasswordErrorMessage").hide();
        ...
        $("#proctor_login_screen").find("#btnProctorLoginSubmit").html("<a class='btn-stem' href='#'>" + proctorLoginScreenObject.ProctorLoginScreen.ProctorLoginScreenSubmitButtonText + "</a></span>");
        ...
        $("#proctor_login_screen").find("#ProctorLoginIDText").html(proctorLoginScreenObject.ProctorLoginScreen.ProctorLoginIDText);
        ...
        $("#proctor_login_screen").find("#ProctorPasswordText").html(proctorLoginScreenObject.ProctorLoginScreen.ProctorPasswordText);
        ...
        $("#proctor_login_screen").find("#btnProctorLoginSubmit").bind('click.namespace', function() {
            ...
            if ((document.getElementById("proctorLogin").value.trim() == "") || (document.getElementById("proctorPassword").value.trim() == "")) {
                if ((document.getElementById("proctorLogin").value.trim() == "")) {
                    $("#proctorLoginErrorMessage").show();
                }

                if ((document.getElementById("proctorPassword").value.trim() == "")) {
                    $("#proctorPasswordErrorMessage").show();
                }
    ...
    this.ShowSpecialPostAssessmentValidation = function(showSpecialPostAssessmentValidationObject) {
        ...
        $("#CAREALSTATEFIELD1ErrorMessage").html("<font color='red'>" + showSpecialPostAssessmentValidationObject.SpecialPostAssessmentValidation.CARealStateRequiredValidationMessage1 + "</font>");
        $("#CAREALSTATEFIELD2ErrorMessage").html("<font color='red'>" + showSpecialPostAssessmentValidationObject.SpecialPostAssessmentValidation.CARealStateRequiredValidationMessage2 + "</font>");
        ...
        $("#CAREALSTATEFIELD1ErrorMessage").hide();
        $("#CAREALSTATEFIELD2ErrorMessage").hide();
        ...
        $("#btnContinueEnd").attr("class","btn-end");
        */
        $("#btnCARealStateValidationContinue").find("a").bind('click.namespace', function() {
            $("#CARealStateValidation").find("#CARealStateErrorLabel").html("");
            $("#CAREALSTATEFIELD1ErrorMessage").hide();
            $("#CAREALSTATEFIELD2ErrorMessage").hide();
            if ((document.getElementById("DRELicenseNumber").value.trim() == "") || (document.getElementById("DriverLicenseNumber").value.trim() == "")) {
                if (document.getElementById("DRELicenseNumber").value.trim() == "") {
                    $("#CAREALSTATEFIELD1ErrorMessage").show();
                }

                if (document.getElementById("DriverLicenseNumber").value.trim() == "") {
                    $("#CAREALSTATEFIELD2ErrorMessage").show();
                }
                return;
            }
        ...
        $("#btnCARealStateValidationCancel").find("a").unbind('click.namespace');
        $("#btnCARealStateValidationCancel").find("a").bind('click.namespace', function() {
    ...
    this.ShowSpecialPostAssessmentAuthenticationResult = function(specialPostAssessmentAuthenticationResultObject) {
        ...
        if (msg == "") {
            ...
            $("#CARealStateValidation").find("#CARealStateErrorLabel").html(msg);
        }
    }
    ...
    this.ShowNYInsuranceValidation = function(showSpecialPostAssessmentValidationObject) {
        ...
        $("#btnNYInsuranceValidationContinue").find("a").bind('click.namespace', function() {
            $("#NYInsuranceValidation").find("#NYInsuranceErrorLabel").html("");
            $("#NYInsuranceFIELDErrorMessage").hide();
            if (document.getElementById("MonitorNumber").value.trim() == "") {
                $("#NYInsuranceFIELDErrorMessage").show();
                return;
            }
        ...
        $("#btnNYInsuranceValidationCancel").find("a").unbind('click.namespace');
        $("#btnNYInsuranceValidationCancel").find("a").bind('click.namespace', function() {
            cp.CancelSpecialPostAssessmentValidation();
        });
    ...
    this.ShowNYInsuranceAuthenticationResult = function(specialPostAssessmentAuthenticationResultObject) {
        ...
        if (msg == "") {
            ...
            $("#NYInsuranceValidation").find("#NYInsuranceErrorLabel").html(msg);
        }
    }
    ...
    function RenderTrueFalseDisabled(arrAnswers) {
        ...
        for (var i = 0; i < arrAnswers.length; i++) {
            ...
            if (arrAnswers[i].Value == "True") {
                dynamicHtml += "<li><input type='radio'  id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' checked='checked' disabled='disabled' onclick='ShowHideNextQuestionButton();'  /><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + RenderTrueFalseString(arrAnswers[i].Value) + "</label></li>";
            }
            else {
                dynamicHtml += "<li><input type='radio'  id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' disabled='disabled' onclick='ShowHideNextQuestionButton();' /><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + RenderTrueFalseString(arrAnswers[i].Value) + "</label></li>";
            }
        }
    ...
    this.RenderTrueFalse = function(arrAnswers, arrStudentAnswers) {
        ...
        if (arrStudentAnswers.length > 0) {
            for (var i = 0; i < arrAnswers.length; i++) {
                ...
                if (arrStudentAnswers[0] == arrAnswers[i].AssessmentItemAnswerID) {
                    dynamicHtml += "<li><input type='radio' checked='checked' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' /><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + RenderTrueFalseString(arrAnswers[i].Value) + "</label></li>";
                }
                else {
                    dynamicHtml += "<li><input type='radio' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' /><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + RenderTrueFalseString(arrAnswers[i].Value) + "</label></li>";
                }
            }
        }
        else {
        ...
        questionCont.html("<ul>" + dynamicHtml + "</ul>");
    ...
    this.RenderSingleSelectMcq = function(arrAnswers, arrStudentAnswers) {
        ...
        if (arrStudentAnswers.length > 0) {
            for (i = 0; i < arrAnswers.length; i++) {
                if (arrStudentAnswers[0] == arrAnswers[i].AssessmentItemAnswerID) {
                    ...
                    //<li><input type="radio" id="one" name="q1" /><label for="one">One</label></li>
                    dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='radio' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "'  /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
                }
            }
        }
        else {
        ...
        var questionCont = $('#question');
        questionCont.html("<ul id='single'>" + dynamicHtml + "</ul>");
        questionCont.find("input[type=radio]").bind('click.namespace', function() {
            ShowHideNextQuestionButton();
        });
        //LCMS-8188 - End
    ...
    this.RenderMultiSelectMcq = function(arrAnswers, arrStudentAnswers) {
        ...
        if (arrStudentAnswers.length > 0) {
            for (var i = 0; i < arrAnswers.length; i++) {
                ...
                dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='checkbox' " + checked + " id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='checkbox' value='" + arrAnswers[i].Value + "' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
                checked = "";
            }
        }
        else {
            for (var i = 0; i < arrAnswers.length; i++) {
                //<li><input type="radio" id="one" name="q1" /><label for="one">One</label></li>
                dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='checkbox' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='checkbox' value='" + arrAnswers[i].Value + "' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
            }
        }
        ...
        var questionCont = $('#question');
        questionCont.html("<ul id='multi'>" + dynamicHtml + "</ul>");
    ...
    this.ShowCourseEvaluationQuestionsMSSQ = function(courseEvalQuestion, questionNo) {
        ...
        if (courseEvalQuestion.Required == true) {
            ...
            dynamicHtml += "<h3 title='" + convertJSString(courseEvalQuestion.Text) + "_Required'>" + '<img style="float:left;display:none" height="14" width="14" alt="" src="images/exclamation.gif" />' + questionNo + ".&nbsp;" + courseEvalQuestion.Text + "&nbsp;" + isRequiredHtml + "</h3>";
        }
        else {
    ...
    this.ShowCourseEvaluationQuestionsFITB = function(courseEvalQuestion, questionNo) {
        ...
        //            }   
        dynamicHtml += "<input type='text' maxlength='256' id='textbox_" + courseEvalQuestion.Id + "' name='textbox' value='' />";
    ...
    this.ShowCourseEvaluationQuestionsTEXT = function(courseEvalQuestion, questionNo) {
        ...
        if (courseEvalQuestion.UnlimitedTF == 1)
            dynamicHtml += "<textarea id=textbox_'" + courseEvalQuestion.Id + "' cols='60' rows='6'></TEXTAREA>";
        else
            dynamicHtml += "<input type='text' maxlength='256' id='textbox_" + courseEvalQuestion.Id + "' name='textbox' value='' />";
    ...
    this.RenderFillInTheBlanks = function(questionStem, arrAnswers, arrStudentAnswersText) {
        ...
        if (arrStudentAnswersText.length > 0) {
            for (var index = 0; index < arrStudentAnswersText.length; index++) {
                ...
                var dynamicHtml = dynamicHtml.replace("&lt;blank&gt;", "<input type='textbox' id='A1' value='" + value + "' >");
            }
            //  var dynamicHtml = questionStem.replace(/<blank>/g,"<input type='textbox' id='1' value='" + value + "' onclick='ShowHideNextQuestionButton();'>");

        }
        else {
            var dynamicHtml = questionStem.replace(/&lt;blank&gt;/g, "<input type='textbox' id='A1' >");
        ...
        //LCMS-8188 - Start
        $('#quiz_container').find('#questionStem').find('input[type=text]').bind('click.namespace', function() {
            ShowHideNextQuestionButton();
        });
        //LCMS-8188 - End
        $('#question').empty();
    ...
    this.ShowQuestionResultRendering = function(objQuestionResult) {
        ...
        if (objQuestionResult.QuestionResult.IsCorrectlyAnswered == true) {
            ...
            if (feedbackDescription != "") {
                ...
                $('#questiondescription').html("<b>" + FeedbackICP4 + ":</b> <br />" + replaceSpacesAndLineBreaks(feedbackDescription));  // Modified by Mustafa On Sep 2nd 2009 //Yasin LCMS-12913      
    ...
    this.ShowProctorMessage = function(objectData) {
        ...
        $('#ShowProctorMessageButtons').find("div").eq(0).html('<div class="floatright"><button class="button">Begin Assessment</button></div>');
        ...
        while (columnCounter < totalNoUl) {
            ...
            for (i = start; i < skippedQuestionLenght; i++) {
                ...
                if (objectSkippedQuestion.SkippedQuestions[i].AssessmentToogleFlag == true) {
                    dynamicHtml += "<img src='\Images\\smallFlag2.png' width='20px' id='ImageButton1'></img>";

                }

                if (objectSkippedQuestion.SkippedQuestions[i].AssessmentToogleFlag == false) {
                    dynamicHtml += "<img src='\Images\\smallFlag1.png' width='20px' id='ImageButton1'></img>";
                }
    ...
    this.RemediationRenderTrueFalse = function(arrAnswers, arrStudentAnswers) {
        ...
        for (i = 0; i < arrAnswers.length; i++) {
            ...
            if (arrAnswers[i].AssessmentItemAnswerID == arrStudentAnswers.AnswerIDs[0]) {
                ...
                dynamicHtml += "<li><input type='radio' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' disabled='disabled' /><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + RenderTrueFalseString(arrAnswers[i].Value) + "</label></li>";
            }
        }
        //$('#question').find('ul').html(dynamicHtml);
        $('#QuestionRemediationQuestion').html("<ul>" + dynamicHtml + "</ul>");
    ...
    this.RemediationRenderSingleSelectMcq = function(arrAnswers, arrStudentAnswers) {
        ...
        for (i = 0; i < arrAnswers.length; i++) {
            if (arrAnswers[i].AssessmentItemAnswerID == selectedMcq) {
                dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='radio' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' disabled='disabled' checked='checked' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
            }
            else {
                dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='radio' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='radioButton' value='" + arrAnswers[i].Value + "' disabled='disabled' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
            }
        }
        //$('#question').find('ul').html(dynamicHtml);
        $('#QuestionRemediationQuestion').html("<ul>" + dynamicHtml + "</ul>");
        ...
        for (i = 0; i < arrAnswers.length; i++) {
            ...
            for (n = 0; n < arrStudentAnswers.AnswerIDs.length; n++) {
                if (arrAnswers[i].AssessmentItemAnswerID == arrStudentAnswers.AnswerIDs[n]) {
                    dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='checkbox' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='checkbox' value='" + arrAnswers[i].Value + "' disabled='disabled' checked='checked' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
                    drawn = true;
                    break;
                }
            ...
            if (drawn == false)
                dynamicHtml += "<li><table style='padding:0px;margin:0px;width:auto;height:auto;'><tr><td valign='top' style='padding-top:5px;'><input type='checkbox' id='" + arrAnswers[i].AssessmentItemAnswerID + "' name='checkbox' value='" + arrAnswers[i].Value + "' disabled='disabled' /></td><td><label for='" + arrAnswers[i].AssessmentItemAnswerID + "'>" + arrAnswers[i].Label + "</label></td></tr></table></li>";
        }
        ...
        */
        if (arrStudentAnswers.AnswerTexts.length > 0) {
            for (var index = 0; index < arrStudentAnswers.AnswerTexts.length; index++) {
                questionStem = questionStem.replace("&lt;blank&gt;", "<input type='textbox' id='" + (index + 1) + "' value='" + arrStudentAnswers.AnswerTexts[index] + "' disabled='disabled'>");
            }
        }
        else {
            questionStem = questionStem.replace(/&lt;blank&gt;/g, "<input type='textbox' id='A1' value=''>");
        }
    ...
    this.RemediationRenderOrdering = function(arrAnswers, arrStudentAnswers) {
        ...
        var dynamicHtml = "<div id='orderselect'><select onchange='javascript:HandleOrderingUpDownButtonsState();' name='ListBox' id='ListBox' size='5'>";
    ...
    this.QuestionRemediationRendering = function(objectQuestions) {
        ...
        document.getElementById('divIncorrect').innerHTML = '<div id="QuestionRemediationIncorrect">' + INCORRECT + '</div>';
        document.getElementById('divCorrect').innerHTML = '<div id="QuestionRemediationCorrect">' + CORRECT + '</div>';
        ...
        $('#QuestionRemediationContainerHeader').find('h1').empty();
        ...
        $('#QuestionRemediationContainerHeader').find('h1').html(queCountStr);
        ...
        if (questionFeedback == "") {
            ...
            $('#QuestionRemediationDescription').html("<b>" + FeedbackICP4 + ":</b> <br />" + questionFeedback);
    ...
    this.ShowDocuSignProcess = function(ShowDocuSignProcessCommandObject) {
        ...
        $(".startDocuSignProcess").bind('click.namespace', function() {
            ...
            window.OnSignCompleted = function(signedStatus) {
                ...
                $(controlPanel).find("#IcoOptions").show();
                $(controlPanel).find("#IcoOptionsDs").hide();

                $(controlPanel).find("#IcoBookMark").show();
                $(controlPanel).find("#IcoBookMarkDs").hide();
...
function parseBookMarkJson(json) {
    if (json.Bookmarks.length > 0) {
        ...
        for (var x in json.Bookmarks) {
            bookMarkStr = bookMarkStr + "<li class=\"enable\"><a href=\"javascript:bookmarkClick('" + json.Bookmarks[x].BookMarkID + "');resetCPIdleTimer();\">" + json.Bookmarks[x].BookMarkTitle + "</a></li>";
        }

        bookMarkStr = bookMarkStr + "</ul>";
        return bookMarkStr;
    }
}
...
function parseCourseMaterialJson(json) {
    if (json.CourseMaterials.length > 0) {
        ...
        courseMaterialStr = courseMaterialStr + "</ul>";
...
function parseGlossaryJson(json) {
    if (json.Glossaries.length > 0) {
        ...
        for (var x in json.Glossaries) {
            ...
            glossaryStr = glossaryStr + "<li title='" + StripTagsCharArray(json.Glossaries[x].Term) + "' class=\"enable\"><a href=\"javascript:glossaryClick('" + json.Glossaries[x].GlossaryID + "','" + InsertEscapeSequence(json.Glossaries[x].Term, "'") + " ');resetCPIdleTimer();\">" + json.Glossaries[x].Term.replace(new RegExp("'", "g"), "&#39").replace(new RegExp("\"", "g"), "&quot;") + "</a></li>";
        ...
        if (!timerRunning) {
            ...
            timerID = setInterval(function() {
                ...
                if (!re.isIdleTimeElapsed) {
                    if ((diffSeconds >= re.whenToShowIdleTimePopup) && (!$("#examIdleTimerDialogue").is(":visible"))) // if timestamp difference reaches to warning time, then show the countdown
                    {
...
SubmitOrdering = function(assesmentId, methodToCall) {
    ...
    var listBox = $('#question').find('select')[0];
    //alert(listBox.options.length);

    for (i = 0; i < listBox.options.length; i++) {
        arrAnswerIds[i] = listBox.options[i].value;
...
SubmitMatching = function(assesmentId, arrAnswers, methodToCall) {
    ...
    for (i = 0; i < totalMatchingBox; i++) {
        arrAnswerIds[i] = $('#drop-' + i).find('span').attr('id');
        arrAnswerStrings[i] = $('#drop-' + i).find('div').find('span').html();
...
function AuthenticateProctor(proctorLogin, proctorPassword) {
    return cp.AuthenticateProctor(proctorLogin, proctorPassword)
}
